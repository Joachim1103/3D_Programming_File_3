...
std::string vs = ShaderLoader::LoadShaderFromFile("Shaders.vs");
std::string fs = ShaderLoader::LoadShaderFromFile("Shaders.fs");
...
void renderLoop(GLFWwindow* window)
{
    ...
    Shader theShader("Shaders.vs", "Shaders.fs");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        GLint viewLoc = theShader.getUniformLocation("view");
        GLint projLoc = theShader.getUniformLocation("projection");
        glUniformMatrix4fv(viewLoc, 1, GL_FALSE, glm::value_ptr(camera.view));
        glUniformMatrix4fv(projLoc, 1, GL_FALSE, glm::value_ptr(camera.projection));

        GLint modelLoc = theShader.getUniformLocation("model");
        GLint colorLoc = theShader.getUniformLocation("objectColor");
        ...
        theShader.setInt("hillTexture", 0);

        // Draw Hill Surface
        glBindVertexArray(hillVAO);
        theShader.setInt("useTexture", 1);
        ...
        for (const auto& position : spherePositions)
        {
            ...
            theShader.setInt("useTexture", 0);
            GLint colorLoc = theShader.getUniformLocation("objectColor");
            glUniform4f(colorLoc, 1.0f, 0.5f, 0.2f, 1.0f); // RGBA Orange
