...
//}
//
//// Generate Hill Vertices with Texture Coordinates
...
//            format = GL_RGBA;
//
//        // Generate texture with the loaded data
...
//{
//    // Example parametric equation: a sine wave
//    float x = 5.0f * t;
//    float y = sinf(t);
//    return glm::vec2(x, y);
//}
//
//// Map a point on the parametric curve to the hill surface using barycentric coordinates
//glm::vec3 mapPointToHill(glm::vec2 point)
//{
//    // Calculate the barycentric coordinates for the point on the hill
//    float x = point.x;
//    float z = point.y;
//    float y = hillFunction(x, z);
//
...
//    glBindVertexArray(0);
//
//    // Generate NPC Cube Vertices and Indices
...
//    glBindVertexArray(0);
//
//    // Generate Parametric Curve Vertices
//    const int numPoints = 100;
//    std::vector<GLfloat> curveVertices;
//    for (int i = 0; i < numPoints; ++i)
//    {
//        float t = static_cast<float>(i) / (numPoints - 1); // Parameter ranging from 0 to 1
...
//    }
//
//    // Parametric Curve Vertex Buffer
...
//        }
//
//        // Calculate character position on the hill using barycentric coordinates
...
//        model = glm::translate(glm::mat4(1.0f), glm::vec3(x, y + 0.25f, z)); // Offset the cube slightly above the hill
//        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
...
//        glBindVertexArray(0);
//
//        // Update NPC position along the parametric curve
//        float oscillatedT = sinf(glfwGetTime() * NPC_SPEED);
//        float normalizedT = (oscillatedT + 1.0f) / 2.0f; // Normalize oscillatedT to [0, 1]
//
//        glm::vec2 parametricPoint = parametricCurve(normalizedT); // Get the point on the parametric curve
//        npcPosition.x = parametricPoint.x; // Use x from the curve
//        npcPosition.z = parametricPoint.y; // Use y from the curve for z-coordinate on the hill
//        npcPosition.y = hillFunction(npcPosition.x, npcPosition.z); // Map to hill surface for y-coordinate
//
//        // Draw NPC (Blue Cube)
//        glBindVertexArray(npcVAO);
//        glUniform1i(glGetUniformLocation(shaderProgram, "useTexture"), 0);
//        glUniform4f(colorLoc, 0.0f, 0.0f, 1.0f, 1.0f); // RGBA blue
//        model = glm::translate(model, npcPosition); // NPC position
//        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
